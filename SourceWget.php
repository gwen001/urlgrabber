...
class SourceWget
{
	...
	public static function run( $target, $tor, $malicious, $https, $params )
	{
		...
		$cmd = 'wget --no-check-certificate --random-wait --user-agent="'.UrlGrabber::T_USER_AGENT[rand(0,UrlGrabber::N_USER_AGENT)].'" -r -l'.$params.' --spider -D '.$domain.' http'.($https?'s':'').'://'.$target.'/ -o '.$tmpfile;
		if( $tor ) {
			$cmd = 'torsocks '.$cmd;
		}
		echo $cmd."\n";
